...
const fetchTopBassHousePlaylist = async () => {
  ...
  const formatTrackList = (trackList) => {
    const formattedList = trackList.map((track, index) => {
      ...
      return `${index + 1}. [${track.track.name}](${track.track.external_urls.spotify}) - ${artists} [![image](${imageUrl})]`;
    }).join("\n");
    return formattedList;
  };

  // Get the top 10 tracks by popularity
  const top10TracksFormated = formatTrackList(sortedTracks.slice(0, 11));
...
const intervalMs = 10 * 1000; // 1 minute
